...
public class UIManager : NetworkBehaviour
{
    ...
    void OnGUI()
    {
        ...
        // Ocupa toda la pantalla con m�rgenes si quieres
        Rect areaGUI = new Rect(10, 10, screenWidth, screenHeight);

        // Puedes usar esto si quieres controlar cu�ndo se muestra el box (igual que en el segundo c�digo)
        bool mostrarBox = !NetworkManager.Singleton.IsClient && !NetworkManager.Singleton.IsServer;
        bool enabled = false;

        if (mostrarBox)
        {
            GUI.Box(areaGUImenu, "Men�");
        }

        GUILayout.BeginArea(mostrarBox ? areaGUImenu : areaGUI);
        GUILayout.Space(mostrarBox ? 40 : 0); // A�ade espacio si es el men�
    ...
    void StartButtons()
    {
        ...
        // Campos de texto para c�digo y nombre
    ...
    */
    ...
    private void GameConfig()
    {
        ...
        if (IsServer || IsHost)
        {
            ...
            // Solo muestra el bot�n "Ready" si ya se ha elegido un modo de juego
    ...
    private void GameHUD()
    {
        ...
        if (GameManager.Instance.endHumanWin.Value)
        {
            ...
            if (IsHost)
            {
                if (GUILayout.Button("Volver al Men�", GUILayout.Width(200)))
                {
